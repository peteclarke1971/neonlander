      ...
      {view === "gameover" && lastResult && (
        <main className="min-h-screen relative flex items-center justify-center">
          ...
          {!needsInitials && showLeaderboardsAfterInitials && lastResult.cause !== "success" && (
            ...
            <nav className="mt-6 flex flex-col items-center gap-2 w-full max-w-xs mx-auto">
              {lastResult.cause === "success" ? (
                mode === "timetrial" ? (
                  <>
                    <button ref={ttRetryRef} className={`player-menu-btn w-full ${goIndex === 0 ? 'selected' : ''}`} onClick={handleRetryLevel} onFocus={() => setGoIndex(0)} disabled={needsInitials}>TRY AGAIN</button>
                    <button ref={ttContRef} className={`player-menu-btn w-full ${goIndex === 1 ? 'selected' : ''}`} onClick={() => handleContinueLevel((lastResult.level ?? 0) + 1)} onFocus={() => setGoIndex(1)} disabled={needsInitials}>CONTINUE</button>
                    <button ref={ttMenuRef} className={`player-menu-btn w-full ${goIndex === 2 ? 'selected' : ''}`} onClick={() => { setView(gameOriginView); resetTimers(); }} onFocus={() => setGoIndex(2)} disabled={needsInitials}>MAIN MENU</button>
                  </>
                ) : (
                  <button ref={contRef} className={`player-menu-btn w-full ${goIndex === 0 ? 'selected' : ''}`} onClick={continueGame}>CONTINUE</button>
                )
              ) : (
                <>
                  <button ref={homeRef} className={`player-menu-btn w-full ${goIndex === 0 ? 'selected' : ''}`} onClick={() => {
                    console.log("🏠 Home button clicked from gameover, returning to:", gameOriginView);
                    setShowLeaderboardsAfterInitials(false);
                    setShowMedleyHighScoreEntry(false);
                    setView(gameOriginView);
                    resetTimers();
                  }} onFocus={() => setGoIndex(0)} disabled={needsInitials || showMedleyHighScoreEntry}>HOME</button>
                  <button ref={retryCurrRef} className={`player-menu-btn w-full ${goIndex === 1 ? 'selected' : ''}`} onClick={retryCurrentLevel} onFocus={() => setGoIndex(1)} disabled={needsInitials || showMedleyHighScoreEntry}>RETRY CURRENT LEVEL</button>
                  <button ref={retryRef} className={`player-menu-btn w-full ${goIndex === 2 ? 'selected' : ''}`} onClick={retryGame} onFocus={() => setGoIndex(2)} disabled={needsInitials || showMedleyHighScoreEntry}>RETRY FROM START</button>
                </>
              )}
            </nav>
          </section>
